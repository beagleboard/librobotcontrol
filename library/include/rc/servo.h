 ...
 *
 *             The Robotics Cape has 8 3-pin headers for connecting hobby servos
 *             and ESCs. These are driven by the PRU for extremely precise
 *             signaling with minimal CPU use. Standard 3-pin servo connectors
 *             are not polarized so pay close attention to the symbols printed
 *             in white silkscreen on the cape before plugging anything in. The
 ...
 *             pulses normally range from 900us to 2100us which usually
 *             corresponds to +- 60 degrees of rotation from the neutral
 *             position. 1500us usually corresponds to the center position. Many
 *             servos work up to +- 90 degrees when given pulse widths in the
 ...
 *             pulse width and can tolerate a wide range of update frequencies.
 *             Servos can typically tolerate update pulses from 5-50hz with more
 *             expensive digital models sometimes capable of higher update
 *             rates. Brushless ESCs are much more tolerant and typically accept
 *             update rates up to 200hz with some multirotor ESCs capable of
 *             400hz when using sufficiently short pulse widths.
 *
 *             Since ESCs drive motor unidirectionally, it makes more sense to
 *             think of their normalized throttle as ranging from 0.0 (stopped)
 *             to 1.0 (full power). Thus, these functions translate a normalized
 *             value from 0.0 to 1.0 to a pulse width between 1000us and 2000us
 *             which is a common factory-calibration range for many ESCs. We
 *             suggest using the rc_calibrate_escs example program on all ESCs
 *             used with the robotics cape to ensure they are calibrated to this
 *             exact pulse range.
 *
 *             We HIGHLY recommend the use of ESCs which use the BLHeli firmware
 *             because this firmware allows the input pulse range to be
 *             programmed to exactly 1000-2000us and the old fashioned
 *             calibration mode to be disabled. This prevents accidental
 *             triggering of calibration mode during use and removes the need to
 *             run rc_calibrate_escs. BLHeli includes a plethora of configurable
 *             settings and features such as easily adjustable timing and
 *             sounds. More information on the BLHeli open source project here.
 *
 *             Unless calibration mode is disabled, most ESCs will go into a
 *             failsafe or calibration mode if the first signals they receive
 *             when powered up are not their calibrated minimum pulse width
 *             corresponding to the throttle-off condition. Therefore it is
 *             necessary for your program to start sending pulses with a
 *             normalized value of 0.0 for a second or more before sending any
 *             other value to ensure expected operation.
 *
 *             Some ESCs, including those running BLHeli firmware, will wake up
 *             but keep the motor idle when receiving pulses slightly below the
 *             minimum. This is largely undocumented but we call this "idle"
 *             mode. For this reason we allow inputs to
 *             rc_send_esc_pulse_normalized and rc_send_esc_pulse_normalized_all
 *             to range from -0.1 to 1.0 where 0.0 results in the lowest
 *             throttle the ESC allows and -0.1 can be used for idle where the
 *             motor is entirely powered off but the ESC is awake.
 *
 *             A recent trend among ESCs is support of "One-Shot" mode which
 *             shrinks the pulse range down to 125-250us for reduced latency.
 *             Like rc_send_esc_pulse_normalized, these oneshot equivalents also
 *             take a range from -0.1 to 1.0 to allow for idle signals.
 *
