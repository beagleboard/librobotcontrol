 ...
 *
 *             Like rc_print_vector, but assumes the contents represent a
 *             polynomial and prints the coefficients with trailing powers of x
 *             for easier reading. This relies on your terminal supporting
 *             unicode UTF-8. numer of coefficients and there the length of
 *             vector v must be less than or equal to 10.
 *
 ...
 *
 *             This finds the coefficients of the polynomials resulting from
 *             multiply a*b. The original contents of c are freed and new memory
 ...
 *             or equal to 0.
 *
 *             Places the result in vector b, any existing memory allocated for
 *             b is freed and its contents are lost. Returns 0 on success and -1
 *             on failure.
 *
 ...
 *             result in c.
 *
 *             Any existing memory allocated for c is freed and its contents are
 *             lost.
 *
 ...
 *
 *             The result is placed in vector a and a's original contents are
 *             lost. More memory is allocated for a if necessary.
 *
 ...
 *
 *             Any existing memory allocated for c is freed and its contents are
 *             lost. Returns 0 on success and -1 on failure.
 *
 * @param[in]  a     First input
 * @param[in]  b     second input
 * @param[out] c     output
 ...
 *
 *             a stays in place and new memory is allocated only if b is longer
 *             than a.
 *
 * @param      a     First input and where output is written
 * @param[in]  b     second input
...
/**
 * @brief      Divides denominator d into numerator n. The remainder is placed into vector
* rem and the divisor is placed into vector div.
 *
 ...
 *             polynomial of order N and cutoff wc (rad/s) and places them in
 *             vector b.
 *
