 ...
 *
 *             All functions have two versions, one that acts on an rc_vector_t
 *             of length 4 and another that acts on normal C arrays, also of
 *             length 4. They are functionally identical and just give the user
 *             options depending on how they represent quaternions in their
 *             code.
 *
 ...
 *
 *             If q is already of length 4 then old contents are simply
 *             overwritten. Otherwise q'd existing memory is freed and new
 *             memory is allocated to aint memory leaks. tb angles are 321 Tait
 *             Bryan angles in array order XYZ with operation order 321(yaw-Z,
 *             pitch-Y, roll-x).
 *
 ...
 * @brief      Calculates quaternion vector q from tait-bryan angles tb.
 *
 *             tb angles are 321 Tait Bryan angles in array order XYZ with
 *             operation order 321(yaw-Z, pitch-Y, roll-x).
 *
 * @param[in]  tb    input tait-bryan angles
 * @param[out] q     output quaternion
 ...
 *
 *             Populates quaternion vector c with the conjugate of quaternion q
 *             where the 3 imaginary parts ijk are multiplied by -1. If c is
 *             already of length 4 then the old values are overwritten.
 *             Otherwise the old memory in c is freed and new memory is
 ...
 *
 *             Populates quaternion vector q with its conjugate where the 3
 ...
 *             quaternion vector q.
 *
 *             If img is already of length 3 then its original contents are
 *             overwritten. Otherwise the original allocated memory is freed and
 ...
 *
 *             Places the result in vector argument c. If c is already of length
 *             4 then the old values are overwritten. Otherwise the old memory
 *             in c is freed and new memory is allocated to help prevent memory
 *             leaks.
 *
 ...
 *             matrix.
 *
 *             q must be normalized before calling this! The orthogonal matrix
 *             corresponds to a rotation by the unit quaternion q when
 *             post-multiplied with a column vector as such: v_rotated=mv.
 *
 *             If m is already 3x3 then its contents are overwritten, otherwise
 *             its existing memory is freed and new memory is allocated.
 *
 * @param[in]  q     The quarter
