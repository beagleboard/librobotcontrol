 ...
 *
 *             This is essential for declaring new ring buffers since structs
 *             declared inside of functions are not necessarily zero'd out which
 *             can cause the struct to contain problematic contents leading to
 *             segfaults. New ring buffers should be initialized with this
 *             before calling rc_ringbuf_alloc.
 *
 ...
 *             rc_ringbuf_t struct.
 *
 *             If buf is already the right size then it is left untouched.
 *             Otherwise any existing memory allocated for buf is freed to avoid
 *             memory leaks and new memory is allocated.
 *
 ...
 *             added to the buffer.
 *
 *             If 'position' is given as 0 then the most recent value is
 *             returned. The position obviously can't be larger than (buffer
 *             size - 1).
 *
 * @param      buf       Pointer to user's buffer
 ...
 *
 *             Note that if the buffer has not yet been filled completely before
 *             calling this, then the starting values of 0.0f in the unfilled
 *             portion of the buffer will still be part of the calculation.
 *
