 ...
 *
 *             Note that using anything other than SCHED_OTHER with priority 0
 *             is only available to root users or after giving special
 *             permission to the executable with the setcap command line
 *             utility.
 *
 *             If a policy is selected which requires privaldges the user does
 *             not have, then a friendly warning will be displayed and the
 *             thread will still be set with the priority and scheduler
 ...
 *
 *             If no timeout is necessary, just use the standard system
 ...
 *
 *             This is just a helpful wrapper for the system getpriority
 *             function and returns the same value.
 *
 ...
 *
 *             Note that this requires special privaledges and will print an
 *             error if run by a normal user. This is just a helpful wrapper for
 *             the system setpriority funtion and returns the same thing.
 *
