 ...
 *
 *             Except for the signal handler setting the state to EXITING, this
 *             is the only way that the process state can be changed.
 *
 ...
 *             you run rc_kill_exising_process() to kill that process. Returns
 *             -1 if there is some other problem writing to the file.
 */
 ...
 *             However, the user may call the rc_kill example program from the
 ...
 *             following:
 *
 *             * SIGINT (ctrl-c) Sets process state to EXITING indicating to the
 *             user threads to shut down cleanly. All user threads should
 *             check rc_get_state to catch this.
 *             * SITERM: Same as SIGINT above
 *             * SIGHUP: Ignored to prevent process from stopping due to loose
 *             USB network connection. Also allows robot control programs to
 *             keep running after USB cable in intentionally removed.
 *             * SIGSEGV:  Segfaults will be caught and print some debugging
 *             info to the screen before setting rc_state to EXITING. Behavior
 *             with segfaults is no guaranteed to be predictable.
 *
 ...
 *             never needed unless you are intentionally changing signal
 *             handlers in the middle of a program.
 *
 * @return     Returns 0 on success or -1 on error
 */
