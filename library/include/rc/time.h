 ...
 *
 *             A wrapper for the normal UNIX nanosleep function which takes a
 *             number of nanoseconds instead of a timeval struct. This also
 *             handles restarting nanosleep with the remaining time in the event
 *             that nanosleep is interrupted by a signal. There is no upper
 *             limit on the time requested.
 *
 ...
 *
 *             The traditional usleep function, however common, is deprecated in
 *             linux as it uses SIGALARM which interferes with alarm and timer
 *             functions. This uses the new POSIX standard nanosleep to
 *             accomplish the same thing which further supports sleeping for
 *             lengths longer than 1 second. This also handles restarting
 *             nanosleep with the remaining time in the event that nanosleep is
 *             interrupted by a signal. There is no upper limit on the time
 *             requested.
 *
 ...
 *             CLOCK_REALTIME
 *
 *             This function itself takes about 1100ns to complete on a
 *             beaglebone at 1ghz under ideal circumstances.
 *
 ...
 *             CLOCK_MONOTONIC
 *
 *             This function itself takes about 1100ns to complete on a bealgebone at 1ghz under
 *             ideal circumstances.
 *
 ...
 *
 *             This uses CLOCK_THREAD_CPUTIME_ID. This time only increments when
 *             the processor is working on the calling thread and not when the
 *             thread is sleeping. This is usually for timing how long blocks of
 *             user-code take to execute. This function itself takes about
 *             2100ns to complete on a beaglebone at 1ghz under ideal
 *             circumstances.
 *
 ...
 *
 *
 *             Convenient for use with nanosleep() function and accurately timed
 *             loops. Unlike timespec_sub defined in time.h, rc_timespec_diff
 *             does not care which came first, A or B. A positive difference in
 *             time is always returned.
 *
 ...
 *
 *             The time added is a double-precision floating point value to make respresenting
 *             fractions of a second easier. The timespec is passed as a pointer
 *             so it can be modified in place. Seconds may be negative.
 *
