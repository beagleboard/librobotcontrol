 ...
 *
 *             It allows use of both dedicated hardware slave-select pins as
 *             well as GPIO pins for slave select. While this was developed on
 *             the BeagleBone platform, it should also work on Raspberry Pi and
 *             other embedded Linux systems.
 *
 *             For the Robotics Cape and BeagleBone blue, the SPI bus 1 is
 *             broken out on two JST SH 6-pin sockets labeled SPI1.1 and SPI1.2
 *             These share clock and serial IO signals, but have independent
 *             slave select lines so two devices can share the same bus. Note
 *             that these ports labeled for slaves 1 and 2 correspond to slaves
 *             0 and 1 in software. To make source code more clear, the macros
 *             RC_BB_SPI1_SS1 and RC_BB_SPI1_SS2 are provided in this header
 *             which are defined as 1,0 and 1,1. the GPIO channels corresponding
 *             to these slave select pins are also provided as macros in this
 *             header.For example:
 *
 * ````
 ...
 *             automatic modes. Only initialize once with either mode.
 *
 ...
 *             select pin.
 *
 *             The provided gpio chip/pin will then be remembered and tied to
 *             the provided slave number and bus. Note that on the BeagleBone
 *             and probably other platforms, there are only two files provided
 *             by the driver for interfacing to the bus, /dev/spi1.0 and
 *             /dev/spi1.1. When using a slave in manual mode, the first
 *             interface (slave 0 in software) will be used to talk to the
 *             manual slaves. Therefore this slave can not be used as an
 *             automatic slave.
 *
 *             For the BeagleBone Blue and RoboticsCape, this will also ensure
 *             that the pinmux is set correctly for that pin. The available
 *             manual slave select pins for these two boards are defined in this
 *             header for convenience. If using other boards it's up to the user
 *             to make sure the pin they are using is set up correctly in the
 *             device tree.
 *
 ...
 *
 * @param[in]  bus       which SPI bus to use
 * @param[in]  slave     slave id
