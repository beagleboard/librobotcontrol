 ...
 *
 *             This is a general-purpose C interface to the linux UART driver
 *             device (/dev/ttyO*). This is developed and tested on the
 *             BeagleBone platform but should work on other linux systems too.
 *
 ...
 *             initialized.
 *
 *             This is so the user can optionally do additional configuration or
 ...
 *
 *             This is a blocking function call. It will only return once the
 *             desired number of bytes has been read from the buffer or the
 *             shutdown flag is set. due to the Sitara's UART FIFO buffer,
 *             MAX_READ_LEN (128bytes) is the largest packet that can be read
 *             with a single call to read(). For reads larger than 128bytes, we
 *             run a loop instead.
 *
 * @param[in]  bus   The bus number /dev/ttyO{bus}
 * @param[out] buf   data pointer
 * @param[in]  byte  number of bytes to read
 *
 ...
 *
 *             This is a blocking function call. It will only return on these
 *             conditions:
